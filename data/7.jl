function fnext(x)
    d = 0.1
    nx1 = x[1] + d * 10 * (x[2] - x[1])
    nx2 = x[2] + d * (x[1] * (28 - x[3]) - x[2])
    nx3 = x[3] + d * (x[1] * x[2] - 8/3 * x[3])
    return [nx1, nx2, nx3]
end
...
ADIM = 3; AMIN = -1.0; AMAX = 1.0; ADEG = 2; ADIV = 2
...
template = x[1]^2 + a[1] * x[2]^2 + a[2] * x[3]^2 + a[3]

# preconditions and postconditions
precond = [x[1]^2 + x[2]^2 + x[3]^2 - 1]
postcond = x[1]^2 + x[2]^2 + x[3]^2 - 4
