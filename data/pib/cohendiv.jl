function fnext(x) 
    ...
    nx3 = x[3] # d is non-deterministically chosen 
    nx4 = x[4] - nx3 * nx2 # r changes 
    nx5 = x[5] + nx3 # q changes 
...
template = x[5] * x[2] + a[1] * x[4] + a[2] * x[1] + a[3]; 

# preconditions and postconditions
precond = [x[4] - x[1], x[1] - x[4], x[5], -x[5]]
postcond = x[5] * x[2] + x[4] - x[1] 

# loopcond: the condition for loop to continue
loopcond = [x[2] - x[4]]

# additional_invariants: invariants which have been proven
additional_invariants = Polynomial{true, Float64}[-x[1]; 1-x[2]; 1-x[3];1 - x[2] * x[3]; -x[4]; -x[5]]
