...
template = 2 * x[1] * x[2] + a[1] * x[3] * x[6] + a[2] * x[4] * x[5] + a[3] 

# preconditions and postconditions
precond = [x[1]-x[3]; x[3]-x[1]; x[2]-x[4]; x[4]-x[2]; x[6]-x[2]; x[2]-x[6]; x[1]-x[5]; x[5]-x[1]] 
postcond = 2 * x[1] * x[2] - x[3] * x[6] - x[4] * x[5] 
...
additional_invariants = Polynomial{true, Float64}[1-x[1]; 1-x[2]; 1-x[3]; 1-x[4]; 1-x[5]; 1-x[6]] 
