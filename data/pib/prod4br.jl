function fnext1(x) 
    nx1 = x[1]; # x does not change 
    nx2 = x[2]; # y does not change 
    nx3 = 1/2 * x[3]; # a changes 
    nx4 = 1/2 * x[4]; # b changes 
    nx5 = 4 * x[5]; # p changes 
    nx6 = x[6]; # q does not change 
...
function fnext2(x) 
    ...
    nx2 = x[2]; # y does not change 
    nx3 = x[3] - 1; # a changes 
    nx4 = x[4]; # b does not change 
    nx5 = x[5]; # p does not change 
    nx6 = x[6] + x[4] * x[5]; # q changes 
    return [nx1, nx2, nx3, nx4, nx5, nx6]; 
end
...
function fnext4(x) 
    ...
    nx4 = x[4] - 1; # b changes 
    nx5 = x[5]; # p does not change 
    nx6 = x[6] + (x[3] + x[4] - 1) * x[5]; # q changes 
    return [nx1, nx2, nx3, nx4, nx5, nx6]; 
end
F_next = [fnext1, fnext2, fnext3, fnext4] 
...
Cond = [2 - x[3]; 2 - x[4]; 2 - x[3]; -1]
# template: template <= 0 is the invariant template
template = x[1] * x[2] + a[1] * x[3] * x[4] * x[5] + a[2] * x[6] + a[3]; 

# preconditions and postconditions
precond = [x[1]-x[3]; x[3]-x[1]; x[2]-x[4]; x[4]-x[2]; 1-x[5]; x[5]-1; -x[6]; x[6]] 
postcond = x[1] * x[2] - x[6] 

# loopcond: the condition for loop to continue
loopcond = [1-x[3]; 1-x[4]] 
notloopcond = [x[3] * x[4]; -x[3] * x[4]] 

# additional_invariants: invariants which have been proven
additional_invariants = Polynomial{true, Float64}[-x[1]; -x[2]; -x[3]; -x[4]; 1-x[5]; -x[6]] 
