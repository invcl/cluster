function fnext(x) 
    nx1 = x[1] + x[5]; # q changes 
    nx2 = x[2]; # D does not change 
    nx3 = x[3] - x[5] * x[2]; # r changes 
    nx4 = x[4]; # N does not change 
    nx5 = 1/2 * x[5]; # p changes 
...
precond = [x[1], -x[1], x[3] - x[4], x[4] - x[3]]
...
additional_invariants = Polynomial{true, Float64}[-x[1]; 1-x[2]; -x[3]; -x[4]; 1-x[5]]
