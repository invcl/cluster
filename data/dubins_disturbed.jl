function fnext_r(x, r)
    d = 0.01 # d is the step size
    w = 1.0178 + 1.8721 * x[1] - 0.0253 * x[2] + r[1] * x[2] # w is the control
    nx1 = x[1] + d * (1 - x[2] * w)
    nx2 = x[2] + d * x[1] * w
    return [nx1, nx2]
end

F_next = [fnext_r]

# XDIM: the number of variables, variables are x[1], ..., x[XDIM]
# XMIN, XMAX: the range of x[i]
# XDEG: the max degree of the invariant (as a polynomial over x[i])
XDIM = 2; XMIN = -100.0; XMAX = 100.0; XDEG = 2

# ADIM: the number of parameters, parameters are a[1], ..., a[ADIM]
# AMIN, AMAX: the range of a[i]
# ADEG: the max degree of p(a) (as a polynomial over a[i])
# ADIV: how many pieces is search range divided into  
ADIM = 3; AMIN = -5.0; AMAX = 5.0; ADEG = 2; ADIV = 10

# RMIN, RMAX: the range of the random disturbance r
RDIM = 1; RMIN = -0.01; RMAX = 0.01;

# rangeA[i], rangeX[i]: range of a[i] and x[i]
rangeA = fill((AMIN, AMAX), ADIM)
rangeX = fill((XMIN, XMAX), XDIM)
rangeR = fill((RMIN, RMAX), RDIM)

@polyvar x[1:XDIM]
@polyvar a[1:ADIM]
@polyvar r[1:RDIM]

# Cond: branching conditions
Cond = [-1]
# template: template <= 0 is the invariant template
template = x[1]^2 + a[1]*x[2]^2 + a[2] * x[2] + a[3]

# preconditions and postconditions
precond = [x[1]^2 + (x[2] - 1)^2 - 1]
postcond = x[1]^2 + (x[2] - 1)^2 - 4

# loopcond: the condition for loop to continue
loopcond = [-1]

# the range of passible initial values, used in simulation
lower_initial = [-1, 0]
upper_initial = [1, 2]

# checking whether the initial value satisfies precondition, used in simulation
function check_pre(x)
    return x[1]^2 + (x[2] - 1)^2 - 1 <= 0 
end

# checking whether the invariant is satisfied, used in simulation
function check_inv(x)
    return x[1]^2 + (x[2] - 1)^2 - 1 <= 0
end

# additional_invariants: invariants which have been proven
additional_invariants = Polynomial{true, Float64}[]
